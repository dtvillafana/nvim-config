...
function Set_Filetype()
    ...
    elseif f:match(".*ansible.*yml")
        or f:match(".*ansible.*inventory$")
        or f:match(".*ansible.*yaml") then
...
function SET_CWD_TO_BUF_DIR()
    ...
    vim.api.exec2(command, {output = false})
end
...
local autocommands = {
    set_custom_extensions_to_bash ={{'BufRead', 'BufNewFile', 'FileType'}, {
        pattern = {'*.curl', '*.wget', '*.aspx', '*.ascx', '*.asmx', '*.config', '*.yaml', '*.yml', 'inventory'},
        callback = function()
            Set_Filetype()
        end
    }},
    set_orgmode_telescope_refiling = {{'FileType'}, {
        pattern = '.*org',
